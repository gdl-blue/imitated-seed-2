...
async function cacheNeededPages() {
	...
	for(var ns of fetchNamespaces()) {
		neededPages[ns] = [];
		var data = await db.all("select distinct link from backlink where exist = '0' and linkns = ?", [ns]);
		for(let i of data)
...
(async function setWikiData() {
	...
	var data = await db.all("select username, perm from perms order by username");
	for(var prm of data) {
		if(permlist[prm.username] == undefined)
	...
	var data = await db.all("select username, key, value from user_settings");
	for(var set of data) {
		if(!userset[set.username])
	...
	switch(Number(config.getString('update_code', '1'))) {
		case 1: {
			// 역링크, 4.2.0 미만용 ACL
			try {
				await db.run("create table backlink (title text default '', namespace text default '', link text default '', linkns text default '', type text default 'link')");
				await db.run("create table classic_acl (title text default '', namespace text default '', blockkorea text default '', blockbot text default '', read text default '', edit text default '', del text default '', discuss text default '', move text default '')");
			} catch(e) {}
		} case 2: {
			// 역링크 테이블에 문서 존재 여부 열 추가
			try {
				await db.run("alter table backlink\nADD exist text;");
			} catch(e) {}
		} case 3: {
			// 문서 테이블에 최종수정일 열 추가
			try {
				await db.run("alter table documents\nADD time text;");
				for(let item of (await db.all("select title, namespace from documents"))) {
					const d = await db.get("select time from history where title = ? and namespace = ? order by cast(rev as integer) desc limit 1", [item.title, item.namespace]);
					if(!d) continue;
					await db.run("update documents set time = ? where title = ? and namespace = ?", [d.time, item.title, item.namespace]);
				}
			} catch(e) {}
		} case 4: {
			// 탈퇴한 사용자
			try {
				await db.run("update res set username = '탈퇴한 사용자' where username = '' and ismember = 'author'");
				await db.run("update history set username = '탈퇴한 사용자' where username = '' and ismember = 'author'");
			} catch(e) {}
		} case 5: {
			// 자동 로그인 구현
			try {
				await db.run("create table autologin_tokens ( username text default '', token text default '' )");
				await db.run("create table trusted_devices ( username text default '', id text default '' )");
			} catch(e) {}
		} case 6: {
			// 로그인 내역 테이블 빼먹음
			try {
				await db.run("alter table login_history\nADD time text;");
			} catch(e) {}
		} case 7: {
			// 위키 설정
			try {
				const fd = await db.get("select value from config where key = 'frontpage'");
				if(fd && fd.value) {
					wikiconfig.front_page = fd.value;
					delete wikiconfig.frontpage;
					await db.run("delete from config where key = 'frontpage' or key = 'front_page'");
					await db.run("insert into config (key, value) values ('front_page', ?)", [fd.value]);
				}
				const cn = await db.get("select value from config where key = 'copyright_notice'");
				if(cn && cn.value) {
					wikiconfig.editagree_text = cn.value;
					delete wikiconfig.copyright_notice;
					await db.run("delete from config where key = 'copyright_notice'");
					await db.run("insert into config (key, value) values ('editagree_text', ?)", [cn.value]);
				}
				for(var key in wikiconfig) {
					if(key == 'update_code') continue;
					await db.run("delete from config where key = ?", [key]);
					await db.run("insert into config (key, value) values (?, ?)", ['wiki.' + key, wikiconfig[key]]);
			...
			// 탈퇴한 사용자 2
			db.run("update history set username = '탈퇴한 사용자', ismember = 'ip' where username = '탈퇴한 사용자' and ismember = 'author'");
			db.run("update res set username = '탈퇴한 사용자', ismember = 'ip' where username = '탈퇴한 사용자' and ismember = 'author'");
			db.run("update block_history set executer = '탈퇴한 사용자', ismember = 'ip' where executer = '탈퇴한 사용자' and ismember = 'author'");
			db.run("update edit_requests set processor = '탈퇴한 사용자', ismember = 'ip' where processor = '탈퇴한 사용자' and ismember = 'author'");
			db.run("update edit_requests set username = '탈퇴한 사용자', ismember = 'ip' where username = '탈퇴한 사용자' and ismember = 'author'");
		} case 9: {
			// 구버전 더시드 토론
			try {
				await db.run("alter table threads\nADD num text;");
				let dd = await db.all("select tnum from threads");
				for(var idx=0; idx<dd.length; idx++) {
					let item = dd[idx];
					let dt = await db.get("select time from res where id = '1' and tnum = ?", [item.tnum]);
					dd[idx].tt = Number(dt.time);
				}
				dd = dd.sort((l, r) => l.tt - r.tt);
				for(var idx=0; idx<dd.length; idx++) {
					let item = dd[idx];
					await db.run("update threads set num = ? where tnum = ?", [String(idx + 1), item.tnum]);
				}
			} catch(e) {}
		} case 10: {
			// 새로운 토론주소
			try {
				await db.run("alter table threads\nADD slug text;");
				await db.run("alter table edit_requests\nADD slug text;");
				var dd = await db.all("select tnum from threads");
				for(let item of dd)
					await db.run("update threads set slug = ? where tnum = ?", [newID(), item.tnum]);
				var dd = await db.all("select id from edit_requests");
				for(let item of dd)
					await db.run("update edit_requests set slug = ? where id = ?", [newID(), item.id]);
			} catch(e) {}
		} case 11: {
			// 까먹음
			try {
				await db.run("alter table res\nADD slug text;");
				var dd = db.all("select tnum from threads");
				for(let item of dd)
					await db.run("update res set slug = ? where tnum = ?", [newID(), item.tnum]);
			} catch(e) {}
		} case 12: {
			// API 토큰
			try {
				await db.run("create table api_tokens (username text default '', token text default '')");
			} catch(e) {}
		} case 13: {
			// API 편집
			try {
				await db.run("alter table history\nADD isapi text;");
			} catch(e) {}
		} case 14: {
			// 더시드 4.16.0이상에서 이상하게 작동하는 버그 수정
			try {
				var dd = await db.all("select id from edit_requests where slug is null or slug = ''");
				for(var item of dd)
					await db.run("update edit_requests set slug = ? where id = ?", [newID(), item.id]);
			} catch(e) {}
		} case 15: {
			try {
				await db.run("alter table aclgroup_groups\nADD css text;");
			...
			try {
				await db.run("create table recover_account (key text default '', username text default '', email text default '', time text default '')");
			} catch(e) {}
		} case 18: {
			try {
				await db.run("alter table aclgroup_groups\nADD disallow_signup text;");
				await db.run("update aclgroup_groups set css = ? where name = ?", ['text-decoration: line-through !important;', '차단된 사용자']);
			} catch(e) {}
		} case 19: {
			try {
				await db.run("alter table files\nADD url text;");
			...
			try {
				await db.run("update aclgroup_groups set css = ? where name = ?", ['text-decoration: line-through !important; color: gray !important;', '차단된 사용자']);
			} catch(e) {}
		} case 23: {
			try {
				await db.run("create table boardipacl (cidr text default '', expiration text default '', note text default '', date text default '')");
				await db.run("create table boardsuspendaccount (username text default '', expiration text default '', note text default '', date text default '')");
			} catch(e) {}
		} case 24: {
			try {
				await db.run("alter table history\nADD loghider text;");
				await db.run("update history set loghider = ''");
			} catch(e) {}
		}
	}
	
	await db.run("update config set value = ? where key = 'update_code'", [updatecode]);
	wikiconfig.update_code = updatecode;
	
	if(hostconfig.debug)
		print('\n경고! 위키가 디버그 모드에서 실행 중입니다. 알려지지 않은 취약점에 노출될 수 있습니다.');
	...
	if(ver('4.18.0')) {
		setInterval(async function clearExpiredAclgroups() {
			var dbdata = await db.all("select username, aclgroup from aclgroup where not expiration = '0' and ? > cast(expiration as integer)", [Number(getTime())]);
		...
		var dbdata = await db.all("select name, css from aclgroup_groups");
		for(var item of dbdata)
			aclgroupCache.css[item.name] = item.css;
		var dbdata = await db.all("select aclgroup, username from aclgroup");
		for(var item of dbdata) {
			if(!aclgroupCache.group[item.username.toLowerCase()])
	...
	if(hostconfig.default_host)
		print('\n포트 ' + (process.env.PORT || 8080) + '번에서 실행 중. . .');
	...
	if(hostconfig.search_autostart)
		child_process.execFile('node', ['search.js'], () => {});
	
	if(hostconfig.file_autostart)
		child_process.execFile('node', ['fileserver.js'], () => {});
})();
