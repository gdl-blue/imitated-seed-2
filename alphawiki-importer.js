...
(async() => {
	...
	print('\n일부 문서만 가져오고 싶다면 제목을 입력해 주세요.');
	print('입력을 다 했거나 입력하지 않고 모두 가져오려면 엔터를 눌러주세요.');
	const docs = [];
	while(1) {
	...
	jsonStream.on('data', (d) => {
		...
		(async() => {
			...
			if(d.value.contributors) for(var item of d.value.contributors) {
				await curs.execute("insert into history (title, namespace, content, rev, username, time, changes, log, ismember, advance) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [title, namespace, d.value.text, rev++, (item.match(/(\d+)[.](\d+)[.](\d+)[.](\d+)/) || item.includes(':') ? item : 'A:' + item), String(new Date().getTime()), '0', 'contributor', (item.match(/(\d+)[.](\d+)[.](\d+)[.](\d+)/) || (item.includes(':') && !item.match(/^.[:]/)) ? 'ip' : 'author'), 'normal']);
			}
			pr++;
		})();
	});
