const { mailer } = require('./functions');

router.all(/^\/member\/recover_password$/, async function(req, res, next) {
    ...
    var content = `\
        ...
        <form method=post class=signup-form>
            ...
            <div class=form-group>
                ${hostconfig.disable_email ? `
                    <strong>이메일이 인증 기능이 비활성화 되었기때문에 사용할 수 없는기능입니다.</strong>
    ...
    if(req.method == 'POST' && !error) {
        ...
        // 사용하면
            ...
            // 메일 발송
            mailer(email, '[' + config.getString('wiki.site_name', '더 시드') + '] ' + username + '님의 아이디/비밀번호 찾기 메일 입니다.',
                `안녕하세요. ${config.getString('wiki.site_name')} 입니다.<br />` + 
                `${username} 님의 아이디/비밀번호 찾기 메일입니다. 해당 계정의 비밀번호를 찾으시려면 아래 링크를 클릭해주세요.<br />` + 
                `<a href="http://${req.hostname}/member/recover_password/${key}">[인증]<br />` + 
                `이 메일은 24시간동안 유효합니다.<br />` + 
                `요청 아이피 : ${ip_check(req)}<br />`);
            // 완료
            return res.send(await render2(req, '계정 찾기', `\
				<p>
    ...
    res.send(await render2(req, '계정 찾기', content, {}, _, error, 'signup'));
});
...
router.all(/^\/member\/recover_password\/(.*)$/, async function(req, res, next) {
	...
	res.send(await render2(req, '계정 찾기', content, {}, _, error, 'recover_password'));
});
