...
router.get(
    ver("4.16.0")
        ? /^\/thread\/([a-zA-Z0-9]+)$/
        : /^\/thread\/([a-zA-Z0-9]{18,24})$/,
    async function viewThread(req, res) {
        var tnum = req.params[0];
        var slug = tnum;
        var data = await curs.execute("select tnum from threads where slug = ?", [
            tnum,
        ]);
        ...
        var data = await curs.execute(
            "select deleted from threads where tnum = ?",
            [tnum],
        );
        ...
        var data = await curs.execute(
            "select title, namespace, topic, status, slug from threads where tnum = ?",
            [tnum],
        );
        ...
        if (aclmsg)
            return res.send(
                await showError(req, { code: "permission_read", msg: aclmsg }),
            );

        var content = `
${ver("4.13.0") ? '<h2 style="font-weight: 700; line-height: 1.1; font-size: 1.8em; margin: 1.2em 0 .8em;">' : '<h2 class="wiki-heading">'}
${html.escape(topic)}
${ver("4.13.0") ? '<label class="pull-right" style = "font-size: 14.2px; font-weight: normal"><input type="checkbox" id="noDisplayHideAuthor" checked></input> 숨겨진 댓글 보이지 않기</label>' : ''}
${getperm("delete_thread", ip_check(req))
                ? '<span class=pull-right><a onclick="return confirm(\'삭제하시겠습니까?\');" href="/admin/thread/' +
                tnum +
                '/delete" class="btn btn-danger btn-sm" style="border-radius:0px; font:13.5px Arial">[ADMIN] 스레드 삭제</a></span>'
                : ""
...
${ver("4.13.0") ? '<div style="border-top: 2px dashed gray;"></div>' :''}
${ver("4.13.0") ? '<h3 style="font-weight: bold; color: inherit; font-family: inherit; font-size: 1.6em; margin: .2em 0 .8em; padding-top: 1em">댓글 달기</h3>' : '<h2 class="wiki-heading">댓글 달기</h2>'}
`;
...
<select name=status style="font: 13.3333px Arial">${sts}</select>
...
${islogin(req) ? "" : `<p style="font-weight: bold; font-size: rem; animation: SAfo-kMM 1s infinite alternate;">[알림] 비로그인 상태로 토론에 참여합니다. 토론 내역에 IP(${ip_check(req)})가 영구히 기록됩니다.</p>`}
...
`;
        ...
        res.send(
            await render(
                req,
                totitle(title, namespace) + " (토론) - " + topic,
                content,
                {
                    document: doc,
                },
...
router.post(
    /^\/thread\/([a-zA-Z0-9]{18,24})$/,
    async function postThreadComment(req, res) {
        var tnum = req.params[0];
        var slug = tnum;
        var data = await curs.execute("select tnum from threads where slug = ?", [
            tnum,
        ...
        if (!req.body["text"])
            return res.status(400).json({
                status: err("error", { code: "validator_required", tag: "text" }) + "",
            });

        var data = await curs.execute(
            "select id from res where tnum = ? order by cast(id as integer) desc limit 1",
            [tnum],
        );
        const lid = Number(data[0].id);

        await curs.execute(
            "insert into res (id, content, username, time, hidden, hider, status, tnum, ismember, isadmin) \
values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [
                String(lid + 1),
                req.body["text"],
                ...
                "0",
                tnum,
                islogin(req) ? "author" : "ip",
                getperm("admin", ip_check(req)) ? "1" : "0",
            ],
        );
        await curs.execute("update threads set time = ? where tnum = ?", [
            getTime(),
            tnum,
        ]);
...
router.get(
    /^\/thread\/([a-zA-Z0-9]{18,24})\/(\d+)\/raw$/,
    async function sendThreadData(req, res) {
        var tnum = req.params[0];
        var slug = tnum;
        var data = await curs.execute("select tnum from threads where slug = ?", [
            tnum,
        ...
        var data = await curs.execute(
            "select username from res where tnum = ? and (id = '1')",
            [tnum],
        );
        ...
        var content = ``;
        var data = await curs.execute(
            "select isadmin, type, id, content, username, time, hidden, hider, status, ismember from res where tnum = ? and id = ? order by cast(id as integer) asc",
            [tnum, Number(tid)],
        );
        ...
        if (
            !data.length ||
            (data[0].hidden == "1" && !getperm("hide_thread_comment", ip_check(req)))
        )
            return res.send("");
...
router.get(
    ...
    async function sendThreadData(req, res) {
        ...
        var content = ``;
        var data = await curs.execute(
            "select isadmin, type, id, content, username, time, hidden, hider, status, ismember from res where tnum = ? and (cast(id as integer) = 1 or (cast(id as integer) >= ? and cast(id as integer) < ?)) order by cast(id as integer) asc",
            [tnum, Number(tid), Number(tid) + 30],
        );
        for (var rs of data) {
            var menu = "";
            if (ver("4.19.0")) {
...
`;
            ...
            if (rs.hidden == "1") {
                ...
                if (getperm("hide_thread_comment", ip_check(req))) {
                    if (ver("4.13.0")) {
                        rescontent +=
                            "<a class=\"btn btn-danger btn-sm\" onclick=\"$(this).parent().attr('class', 'r-body'); $(this).parent().html($(this).parent().children('.hidden-content').html()); return false;\">[ADMIN] SHOW</a><div class=hidden-content style=\"display:none\">" +
                            rc +
                            "</div>";
                    } else {
                        rescontent +=
                            '<div class=text-line-break style="margin: 25px 0px 0px -10px; display:block"><a class=text onclick="$(this).parent().parent().children(\'.hidden-content\').show(); $(this).parent().css(\'margin\', \'15px 0 15px -10px\'); $(this).hide(); return false;" style="display: block; color: #fff;">[ADMIN] Show hidden content</a><div class=line></div></div><div class=hidden-content style="display:none">' +
                            rc +
                            "</div>";
                    }
                }
...
<span class=num>
<a id="${rs.id}">#${rs.id}</a>&nbsp;
</span>${ip_pas(rs.username_disbled, rs.ismember, 1).replace("<a ", rs.isadmin == "1" ? '<a style="font-weight: bold;" ' : "<a ")}${rs["ismember"] == "author" && (await userblocked(rs.username))
                    ? `<del><a href="/w/사용자:${rs.username}" style="color: gray">${rs.username}</a></del>`.replace(
                        "<a ",
                        rs.isadmin == "1" ? '<a style="font-weight: bold;" ' : "<a ",
                    )
                    : `<a href="/w/사용자:${rs.username}">${rs.username}</a>`.replace(
                        "<a ",
                        rs.isadmin == "1" ? '<a style="font-weight: bold;" ' : "<a ",
                    )
...
router.get(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/(\d+)\/show$/,
    async function showHiddenComment(req, res) {
        var tnum = req.params[0];
        var slug = tnum;
        var data = await curs.execute("select tnum from threads where slug = ?", [
            tnum,
        ...
        await curs.execute(
            "update res set hidden = '0', hider = '' where tnum = ? and id = ?",
            [tnum, tid],
        );
...
router.get(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/(\d+)\/hide$/,
    async function hideComment(req, res) {
        ...
        if (!getperm("hide_thread_comment", ip_check(req)))
            return res.send(await showError(req, "permission"));
        await curs.execute(
            "update res set hidden = '1', hider = ? where tnum = ? and id = ?",
            [ip_check(req), tnum, tid],
        );
...
router.post(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/status$/,
    async function updateThreadStatus(req, res) {
        ...
        if (!["close", "pause", "normal"].includes(newstatus))
            res.status(400).send("");

        await curs.execute(
            "update threads set time = ?, status = ? where tnum = ?",
            [getTime(), newstatus, tnum],
        );
        await curs.execute(
            "insert into res (id, content, username, time, hidden, hider, status, tnum, ismember, isadmin, type) \
values (?, ?, ?, ?, '0', '', '1', ?, ?, ?, 'status')",
            [
                String(rescount + 1),
                newstatus,
...
router.post(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/document$/,
    async function updateThreadDocument(req, res) {
        ...
        await curs.execute(
            "update threads set time = ?, title = ?, namespace = ? where tnum = ?",
            [getTime(), dd.title, dd.namespace, tnum],
        );
        await curs.execute(
            "insert into res (id, content, username, time, hidden, hider, status, tnum, ismember, isadmin, type) \
values (?, ?, ?, ?, '0', '', '1', ?, ?, ?, 'document')",
...
router.post(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/topic$/,
    async function updateThreadTopic(req, res) {
        ...
        await curs.execute(
            "update threads set time = ?, topic = ? where tnum = ?",
            [getTime(), newtopic, tnum],
        );
        await curs.execute(
            "insert into res (id, content, username, time, hidden, hider, status, tnum, ismember, isadmin, type) \
values (?, ?, ?, ?, '0', '', '1', ?, ?, ?, 'topic')",
...
router.get(
    /^\/admin\/thread\/([a-zA-Z0-9]{18,24})\/delete/,
    async function deleteThread(req, res) {
        ...
        var data = await curs.execute(
            "select title, namespace from threads where tnum = ?",
            [tnum],
        );
        const title = totitle(data[0].title, data[0].namespace) + "";

        await curs.execute("update threads set deleted = '1' where tnum = ?", [
            tnum,
        ]);
...
router.post(
    ...
    async function notifyEvent(req, res) {
        ...
        if (!rescount) return res.send(await showError(req, "thread_not_found"));
        var data = await curs.execute(
            "select id from res where tnum = ? order by cast(time as integer) desc limit 1",
            [tnum],
        );
        res.json({
            status: "event",
