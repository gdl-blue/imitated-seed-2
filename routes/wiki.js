...
router.get(/^\/w\/(.*)/, async function viewDocument(req, res) {
	...
	if(aclmsg) {
		...
		var data = await curs.execute("select flags, rev, time, changes, log, iserq, erqnum, advance, ismember, username, loghider from history where title = ? and namespace = ? order by cast(rev as integer) desc limit 3",
						[doc.title, doc.namespace]);
		
		return res.status(404).send(await render(req, totitle(doc.title, doc.namespace), 'notfound', {
			document: doc,
			user: doc.namespace == '사용자',
			discuss_progress: !!dpg.length,
			editable: ver('4.22.7') ? editable : undefined,
			edit_acl_message: ver('4.22.7') ? edit_acl_message : undefined,
			history: data.length ? data : null,
		}));
	} else {
		if(rawContent[0].content.startsWith('#redirect ')) {
			...
			content = await namumark(req, rawContent[0].content, 0, doc + '');
		}
