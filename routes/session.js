router.get(/^\/member\/logout$/, async(req, res, next) => {
	...
	if(autologin) {
		db.run("delete from autologin_tokens where token = ?", [sha3(autologin)]);
...
router.all(/^\/member\/login$/, async function loginScreen(req, res, next) {
	...
	if(req.method == 'POST') do {
		...
		if(!id) break;
		var usr = await db.get("select username from users where lower(username) = ?", [id.toLowerCase()]);
		var invalidusername = !id || !usr;
		if(invalidusername) break;
		if(!pw) break;
		var data = await db.get("select username, password from users where lower(username) = ? and password = ?", [id.toLowerCase(), sha3(pw)]);
		var invalidpw = !invalidusername && (!data || !pw);
		if(invalidpw) break;
		var blocked = ver('4.1.0') ? false : await userblocked(id);
		if(blocked) break;
	} while(0);
	...
	var content = `\
		...
		<form class=login-form method=post>
			<div class=form-group>
				...
				${req.method == 'POST' && !error && !id ? (error = err('p', { code: 'validator_required', tag: 'username' })) : ''}
	...
	if(req.method == 'POST' && !error) {
		...
		if(req.body['autologin']) {
			...
			res.cookie('honoka', key, {
				...
				httpOnly: true,
				secure: hostconfig.sessionhttps || false,
				samesite: 'lax',
			});
			await db.run("insert into autologin_tokens (username, token) values (?, ?)", [id, sha3(key)]);
		}
		
		if(!hostconfig.disable_login_history) {
			db.run("insert into login_history (username, ip, time) values (?, ?, ?)", [id, getperm('hideip', id) ? '127.0.0.1' :  ip_check(req, 1), getTime()]);
			db.run("delete from useragents where username = ?", [id]).then(() => {
				db.run("insert into useragents (username, string) values (?, ?)", [id, req.headers['user-agent'] || '']);
			});
		}
