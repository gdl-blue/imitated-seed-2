...
router.all(/^\/edit\/(.*)/, async function editDocument(req, res, next) {
	...
	var error = null;
	var content = '';
	var section = parseInt(req.query['section']) || null;
	var baserev = 0;
	...
	if(!req.session.edit_count) req.session.edit_count = 0;
	...
	if(aclmsg) {
		...
		content = error + content.replace('&<$TEXTAREA>', textarea).replace('<textarea', '<textarea readonly=readonly') + `</form>`;
		httpstat = 403;
	} else {
		content += `\
	...
	if(!aclmsg && req.method == 'POST') do {
		if(((islogin(req) && req.session.edit_count % 20 == 0) || req.session.edit_count % 10 == 0) && !validateCaptcha(req, true)) { content = (error = err('alert', { code: 'captcha_validation_failed' })) + content; break; }
		...
		req.session.edit_count++;
		if(req.session.edit_count > 20) req.session.edit_count = 1;
