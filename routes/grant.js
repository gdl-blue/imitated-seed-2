router.all(/^\/admin\/grant$/, async(req, res, next) => {
	...
	var content = `
		...
		<br />
	`;
	if(username === undefined) return res.send(await render2(req, '권한 부여', content, {}, _, error, 'grant'));
	if(!username) return res.send(await render2(req, '권한 부여', (error = err('alert', { code: 'validator_required', tag: 'username' })) + content, {}, _, error, 'grant'));
	...
	if(req.method == 'POST') {
		...
		for(var prm of perms) {
			...
			} else if(!getperm(prm, username, true) && (typeof(prmval.find(item => item == prm)) != 'undefined')) {
				logstring += '+' + (ver('4.20.0') && item == 'no_force_recaptcha' ? 'no_force_captcha' : prm) + ' ';
		...
		if(!logstring.length)
			return res.send(await render2(req, '권한 부여', (error = err('alert', { code: 'no_change' })) + content, {}, _, error, 'grant'));
	...
	res.send(await render2(req, '권한 부여', content, {}, _, _, 'grant'));
});
